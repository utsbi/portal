...
export function makeChain(initial: { data: unknown; error: unknown } = { data: null, error: null }): ChainMock {
...
export function makeSupabaseMock(): {
  ...
  setTableResult: (table: string, result: { data: unknown; error: unknown }) => void;
  ...
  function setUser(user: unknown) {
    mockSupabase.auth.getUser.mockResolvedValue({ data: { user }, error: user ? null : { message: "Not authenticated" } });
  }
