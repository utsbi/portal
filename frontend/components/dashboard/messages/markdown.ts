...
function expandTextWithLinks(token: Token, keyPrefix: string): ReactNode[] {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (m.index > cursor) {
      nodes.push(
        createElement("span", { key: `${keyPrefix}-pre${i}` }, token.content.slice(cursor, m.index)),
      );
    }
    nodes.push(
      createElement(
  ...
  if (cursor < token.content.length) {
    nodes.push(
      createElement("span", { key: `${keyPrefix}-suf` }, token.content.slice(cursor)),
    );
  }
  return nodes;
}
  ...
  switch (token.type) {
    ...
    case "code":
      return createElement(
        "code",
        { key, className: "px-1 py-0.5 rounded bg-sbi-dark/60 font-mono text-[12px]" },
        token.content,
      );
    case "link":
      return createElement(
...
function highlightText(
  ...
  while (idx !== -1) {
    if (idx > cursor) {
      nodes.push(createElement(Fragment, { key: `${keyPrefix}-pre${matchCount}` }, text.slice(cursor, idx)));
    }
    nodes.push(
      createElement(
        "mark",
  ...
  if (cursor < text.length) {
    nodes.push(createElement(Fragment, { key: `${keyPrefix}-suf${matchCount}` }, text.slice(cursor)));
  }
  return nodes;
}

/** Walk rendered nodes and inject highlight marks into text-level nodes. */
function injectHighlight(nodes: ReactNode[], highlight: string, active: boolean, keyPrefix: string): ReactNode[] {
  if (!highlight) return nodes;
  return nodes.map((node, i) => {
    // Bare string (text token with no URLs).
    if (typeof node === "string") {
      const parts = highlightText(node, highlight, `${keyPrefix}-n${i}`, active);
    ...
    if (isValidElement(node)) {
      ...
      const el = node as { key: string | null; type: unknown; props: { children?: ReactNode; className?: string; [k: string]: unknown } };
      const child = el.props.children;
      if (typeof child === "string") {
        const parts = highlightText(child, highlight, `${keyPrefix}-el${i}`, active);
        return createElement(
          el.type as string,
...
export function renderMarkdown(text: string, opts?: RenderMarkdownOpts): ReactNode {
  if (!text) return null;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (h1Match) {
      ...
      nodes.push(
        createElement(
          ...
          highlight ? injectHighlight(content, highlight, highlightActive, `h1inj-${i}`) : content,
        ),
      );
      if (i < lines.length - 1) nodes.push(createElement("br", { key: `br-${i}` }));
      continue;
    }
    ...
    if (h2Match) {
      ...
      nodes.push(
        createElement(
          ...
          highlight ? injectHighlight(content, highlight, highlightActive, `h2inj-${i}`) : content,
        ),
      );
      if (i < lines.length - 1) nodes.push(createElement("br", { key: `br-${i}` }));
      continue;
    ...
    if (h3Match) {
      ...
      nodes.push(
        createElement(
          ...
          highlight ? injectHighlight(content, highlight, highlightActive, `h3inj-${i}`) : content,
        ),
      );
      if (i < lines.length - 1) nodes.push(createElement("br", { key: `br-${i}` }));
      continue;
    ...
    const finalNodes = highlight ? injectHighlight(inlineNodes, highlight, highlightActive, `inj-${i}`) : inlineNodes;
