...
│       ├── agents/
│       │   ├── __init__.py      # Package exports (run_explore_agent, streaming, OpenRouterClient)
│       │   ├── explore.py       # Agent entry points + OpenRouterClient
│       │   ├── graph.py         # LangGraph state machine + SSE streaming generator
│       │   ├── nodes.py         # Agent workflow nodes (rewrite, route, retrieve, generate, format)
...
│       └── services/
│           ├── __init__.py
│           ├── google_cal.py    # Google Calendar service
│           ├── pdf_parser.py    # PDF text extraction
│           └── rag_service.py   # RAG operations (embeddings, search, context)
└── claude.md                    # Project context
```

**Import convention:** All internal imports use the `app.explore.` prefix (e.g., `from app.explore.core.config import settings`).

## Key Components

### 1. LangGraph Agent (`agents/graph.py`)

The agent uses a state machine with five node functions, split for SSE phase granularity:

1. **Rewrite Query** (`nodes.py:rewrite_query`): Greeting/help detection and query rewriting. Uses `FAST_MODEL` to rewrite vague follow-up queries into standalone search terms when conversation history exists. Greetings and help requests are fast-pathed to direct response.
2. **Semantic Route** (`nodes.py:semantic_route`): Uses the **original query** (not rewritten) to examine intent against file name + content preview (500 chars) and knowledge base description. Classifies the route as ATTACHMENT, RAG, or HYBRID. Defaults to HYBRID when ambiguous. Uses `FAST_MODEL`. When no attachments exist, defaults to RAG without an extra LLM call.
3. **Retrieve Context** (`nodes.py:retrieve_context`): Fetches context based on the routing decision:
...
4. **Generate Response** (`nodes.py:generate_response`): Creates the answer using LLM via OpenRouter with Markdown formatting
5. **Format Sources** (`nodes.py:format_sources`): Prepares source citations for the response

**Compiled Graph Path:** `route_query()` wraps both `rewrite_query()` + `semantic_route()` for the non-streaming compiled graph path.

**SSE Streaming Path:** `run_graph_streaming()` calls the split functions individually, yielding phase events between each node execution:
...
| --------------------------- | ------ | ------------------------------------------- |
| `/api/v1/chat/`             | POST   | Chat with AI agent (SSE streaming response) |
...
The chat endpoint (`POST /api/v1/chat/`) returns a `StreamingResponse` with `text/event-stream` media type instead of a single JSON response.
...
data: {"type": "result", "answer": "...", "sources": [...], "route": "...", "route_reason": "..."}
...
**Client disconnection:** The endpoint checks `raw_request.is_disconnected()` between events and stops streaming if the client has disconnected.
...
python -m uvicorn app.explore.main:app --reload --port 8000
```
...
The response is delivered as a Server-Sent Events stream. Phase events arrive as each backend node executes, followed by the final result:
...
data: {"type": "result", "answer": "Based on the project documentation...", "sources": [{"content": "Excerpt...", "filename": "project_plan.pdf", "page_number": 5, "relevance_score": 0.87}], "route": "retrieve", "route_reason": "No attachments, default RAG search"}
...
- **SSE Streaming Progress**: Loading phases (thinking, planning, searching, generating) are driven by real-time backend SSE events, not timers
...
| `lib/api/chat.ts`                                  | API client: SSE stream parsing, sendChatMessage, extractFileText, upload  |
| `components/dashboard/explore/ui/PortalInput.tsx`  | Chat input with file upload, model picker, send/stop button, animated prop|
| `components/dashboard/explore/ui/ChatMessage.tsx`  | Message rendering: markdown, sources, edit, copy, redo, cancel state     |
| `components/dashboard/explore/ui/ChatMessages.tsx` | Message list container with auto-scroll via scrollIntoView               |
| `components/dashboard/explore/ui/ChatLoading.tsx`  | Loading indicator with phase labels and progress bar (SSE-driven)        |
| `components/dashboard/explore/DashboardPortal.tsx` | Page layout, GSAP animations, full-width scroll container, session cleanup|

### Backend Endpoint Details

- `chat.py`: Runs the LangGraph agent via `run_explore_agent_streaming`, yielding SSE phase events. Monitors client disconnection via `raw_request.is_disconnected()` to stop streaming if the user navigates away or hits stop
...
2. **Frontend**: `streamText` checks `cancelledRef` on each tick and resolves immediately if cancelled
3. **Frontend**: If streaming, preserves `displayedContent` as final content and marks message as `isCancelled`
4. **Backend**: `chat.py` checks `raw_request.is_disconnected()` between SSE events - when the client disconnects (abort), streaming stops
5. **Backend**: All LLM calls are async (`AsyncOpenAI`), so cancellation propagates and stops token generation

## How SSE Phase Streaming Works (Frontend <-> Backend)

1. **Backend**: `run_graph_streaming()` in `graph.py` is an async generator that yields `{"type": "phase", "phase": "..."}` events between node executions
2. **Backend**: `chat.py` wraps this in a `StreamingResponse` with `text/event-stream` media type
3. **Frontend**: `sendChatMessage()` in `chat.ts` reads the SSE stream via `response.body.getReader()`, parses each `data:` line
4. **Frontend**: Phase events trigger the `onPhase` callback, which updates `loadingPhase` state in `chat-context.tsx`
5. **Frontend**: `ChatLoading.tsx` maps `loadingPhase` to labels and progress bar widths (thinking=20%, planning=45%, searching=70%, generating=90%)
